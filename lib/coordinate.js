 ...
 * @description constructor function to generate a GeoCoordinate
 */
...
function parseCoordinateArray(coordinate) {
  if (
    ...
    throw new TypeError(
      'Invalid array passed to GeoCoordinate constructor: [' +
        coordinate.join(', ') +
        ']'
    );
  }
...
function assertNotEmpty(obj) {
  if (obj === undefined || obj === null) {
    throw new TypeError(
      'Empty value passed to GeoCoordinate constructor: ' + obj
    );
  }
}
...
function parseCoordinateObject(coordinate) {
  ...
  const err = new TypeError(
    'Invalid coordinate object passed to GeoCoordinate constructor'
  );
...
function isCoordinateObject(coordinate) {
  return (
    Object.hasOwnProperty.call(coordinate, 'latitude') &&
    Object.hasOwnProperty.call(coordinate, 'longitude')
  );
}
...
GeoCoordinate.prototype.preciseDistanceTo = function(coordinate) {
  const longitudeDelta =
    Math.abs(coordinate.longitude() - this.longitude()) * D2R;
  const latitudeDelta = Math.abs(coordinate.latitude() - this.latitude()) * D2R;
  const a =
    Math.pow(Math.sin(latitudeDelta / 2), 2) +
    Math.cos(this.latitude() * D2R) *
      Math.cos(coordinate.latitude() * D2R) *
