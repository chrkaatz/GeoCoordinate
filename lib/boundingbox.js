 ...
 * GeoBoundingBox
 * @constructor
 * @description constructor function to generate a GeoBoundingBox
 */
...
GeoBoundingBox.prototype.pushCoordinate = function(latitude, longitude) {
  if (this.settled) {
    throw new Error(
      'Instances created by static method .fromCoordinates are locked'
    );
  }
...
GeoBoundingBox.prototype.contains = function(latitude, longitude) {
  ...
  const doesNotContain = this.axes.some(axis => {
    if (isNaN(axis.min) || isNaN(axis.max)) {
      return true;
    }
