...
function smallestCircularDistance(range, a, b) {
    ...
    let categoryA = Math.round(a / halfRange) / 2;
    let categoryB = Math.round(b / halfRange) / 2;
    let normalizeAmount = Math.floor(categoryA - categoryB);
...
GeoBoundingBox.fromCoordinates = function() {
    ...
    for(let i = 0; i < arguments.length; i++) {
        instance.pushCoordinate(argument[i]);
    }
...
GeoBoundingBox.prototype.pushCoordinate = function (latitude, longitude) {
    ...
    this.axes.forEach(function (axis) {
        ...
        let distanceFromMin = smallestCircularDistance(axis.range, axis.min, axis.value);
        let    distanceFromMax = smallestCircularDistance(axis.range, axis.max, axis.value);
