...
module.exports = testCase({
    "test distance implementations": function(test){
        for (var i=0; i < goldDist.length; i++){
            ...
            equalDecimal(test, p1.quickDistanceTo(p2), testData[4], testData[4] * 0.015, "too big deviation (quickDistanceTo)");
        }
        test.done();
    },
    ...
    "test get bearing almost north 2": function(test) {
        const north = new GeoCoordinate([52.50, 13.00, 0]);
    ...
    "test get altitude": function(test) {
        const bagdad = new GeoCoordinate([35,45,120]);
        equalDecimal(test, bagdad.altitude(), 120, 0, 'Bagdad altitude doesn\'t match');
        const randomPoint = new GeoCoordinate([40,24]);
        equalDecimal(test, randomPoint.altitude(), 0, 0, 'Default altitude isn\'t 0');
        test.done();
    },
