...
module.exports = testCase({
  "test bounding box taking smallest possible box around 180th meridian": function(
    test
  ) {
    ...
    test.ok(
      bb.contains(box.topLeftLatitude, box.topLeftLongitude),
      "bounding box should contain boundary coordinate"
    );
    test.ok(
      !bb.contains(26.9, 154.8),
  ...
  "test mergeBox": function(test) {
    ...
    test.strictEqual(
      expectedNewBox.topLeftLatitude,
      box.topLeftLatitude,
      "topLeftLatitude not as expected"
    );
    test.strictEqual(
      expectedNewBox.topLeftLongitude,
      box.topLeftLongitude,
      "topLeftLongitude not as expected"
    );
    test.strictEqual(
      expectedNewBox.bottomRightLatitude,
      box.bottomRightLatitude,
      "bottomRightLatitude not as expected"
    );
    test.strictEqual(
      expectedNewBox.bottomRightLongitude,
      box.bottomRightLongitude,
      "bottomRightLongitude not as expected"
    );
  ...
  "test box containing circle": function(test) {
    ...
    test.ok(
      bb.contains(centrePoint.pointAtDistance(250, 0)),
      "should contain a point 250 metres from the centre"
    );
    test.ok(
      bb.contains(centrePoint.pointAtDistance(750, 180)),
      "should contain a point 750 metres from the centre"
    );
    test.ok(
      false === bb.contains(centrePoint.pointAtDistance(1750, 180)),
